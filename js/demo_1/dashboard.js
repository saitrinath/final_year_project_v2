(function ($) {
  ...
  $(function () {
    ...
    if ($('#sales-statistics-overview').length) {
      ...
      document.getElementById('sales-statistics-legend').innerHTML = salesChart.generateLegend();
		async function update_chart(){
			...
			//get data
				  await fetch('http://192.168.43.75:5000/API/sensors/am2302_history')
			.then(res => res.json())
			.then((out) => {
				console.log('am2302_history: received');
		  
			var data_5_1 = out['temp'];
			var data_5_2 = out['hum'];
			var data_5_3 = out['time'];
						var data = salesChart.data;
        data.datasets[0].data = data_5_2;
        data.datasets[1].data = data_5_1;
		data.labels=data_5_3;
        salesChart.update();
			//console.log('data5',data_5_1,data_5_2,data_5_3);
		}).catch(err => console.error(err));
		
		//update chart			
		}
		fetch('http://192.168.43.75:5000/API/sensors/am2302_history')
			.then(res => res.json())
			.then((out) => {
		...
		//console.log("check");
		var sss=setInterval(update_chart,10000);
    ...
    async function update_guage(){
			
			//get data
				  await fetch('http://192.168.43.75:5000/API/sensors/am2302_thi')
			.then(res => res.json())
			.then((out) => {
				console.log('am2302_thi : received');
		...
		var sss=setInterval(update_guage,10000);
